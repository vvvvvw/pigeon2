...
public class StringizerUtils {
	...
	public static enum JsonStringizer {
		...
		private void fromObject(Set<Object> done, LengthLimiter sb, Object obj) {
			...
			if (type == String.class) {
				...
				sb.append("{}");  //如果在done中已经存在obj，//?则sb.append("{}")
				return;
			} else {
				done.add(obj);   //？如果是在上面处理不掉的复杂类型，将该obj添加到done中
	...
	public static class Reflects {
		...
		public static enum MethodReflector {
			...
			//是否是get方法，get方法是public且不是static的
		...
		public static enum ResourceReflector {
			...
			//如果resName不以/开头，则返回与anchorClass同包的resName文件中的Properties
			//否则，返回resName对应的属性文件中的属性
			...
			//如果resName不以/开头，则返回与Reflects.class同包的resName文件中的Properties
			//否则，返回resName对应的属性文件中的属性
			public Properties getResource(String resName) {
				return getResource(Reflects.class, resName);
			}

			//如果resName以/开头，将/去除后返回resName
			//否则，获取clazz的非限定类名，如果传入clazz为空，获取本类的非限定类名
			//返回与clazz包目录下的resName文件
