...
public class AnnotationBean implements DisposableBean, BeanFactoryPostProcessor, BeanPostProcessor,
    ...
    //在annotationPackage对应的包中扫描@Service注解的类
	...
	//?如果配置了annotationPackages，一定要在annotationPackages下么？
	public Object postProcessAfterInitialization(Object bean, String beanName) throws BeansException {
		...
		postProcessBeforeInitialization(bean, beanName); //？这个调用关系不太懂
		return bean;
	}
			...
			if (name.length() > 3 && name.startsWith("set") && method.getParameterTypes().length == 1
				...
				try {
					...
					if (reference != null) {
						...
						if (value != null) {
							method.invoke(bean, new Object[] {}); //?这边传入new Object[] {}是不是错了，参照下面
						}
					}
				} catch (Throwable e) {
	...
	private Object refer(Reference reference, Class<?> referenceClass) { // method.getParameterTypes()[0]
		...
		//key：${group}/${url}@${interfaceName}:${version}:${serialize}:${protocol}:${timeout}:${callType}
		...
		if (invokerConfig == null) {
			...
			invokerConfig.setSecret(reference.serialize());
	...
	//如果没有配置annotationPackages，返回true
